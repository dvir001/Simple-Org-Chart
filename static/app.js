...
async function exportToPDF(exportFullChart = false) {
    ...
    try {
        ...
        // --- Metadata page (appended as last page) ---
        pdf.addPage();

        const metaX = margin;
        let metaY = margin + 10;

        // Try to add the configured logo
        try {
            const logoSrc = (appSettings && appSettings.logoPath) ? appSettings.logoPath : '/static/icon.png';
            const logoDataUrl = await imageToDataUrl(logoSrc);
            if (logoDataUrl) {
                pdf.addImage(logoDataUrl, 'PNG', metaX, margin, 15, 15);
                // Title next to logo
                pdf.setFontSize(16);
                pdf.setTextColor(0, 0, 0);
                pdf.setFont(undefined, 'bold');
                pdf.text('Simple Org Chart', metaX + 18, margin + 6);
                pdf.setFontSize(12);
                pdf.text('Export Report', metaX + 18, margin + 12);
                metaY = margin + 22;
            } else {
            ...
            pdf.setTextColor(0, 0, 0);
            pdf.setFont(undefined, 'bold');
            pdf.text('Simple Org Chart', metaX, metaY);
            metaY += 6;
            pdf.setFontSize(12);
            pdf.text('Export Report', metaX, metaY);
            metaY += 12;
        }
