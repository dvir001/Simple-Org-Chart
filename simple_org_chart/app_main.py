...
def _sanitize_contact_number(value):
...
def _split_name_parts(display_name):
...
def _parse_custom_directory_contacts(raw_text):
    contacts = []
    if not raw_text:
        return contacts
...
def _build_microsip_directory_items(employees, *, settings=None):
...
def get_microsip_directory():
    ...
    settings = load_settings()
    items = _build_microsip_directory_items(employees, settings=settings)
    payload = {
